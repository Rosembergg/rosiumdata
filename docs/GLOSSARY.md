...
> **Vocabulário do projeto.** Definições precisas de todos os termos, conceitos e jargões usados na RSdata.  
...
### Adapter
Peça que traduz o mundo externo (API REST, GraphQL, array local, banco de dados) para o formato que o Core entende. É a **fronteira** entre a RSdata e qualquer fonte de dados. Todo Adapter implementa a mesma interface (`DataAdapter`), independente da fonte. É o conceito que torna a RSdata universal: trocou de backend, troca o adapter; a tabela inteira continua idêntica.
...
### Dado plano (flat)
Uma linha de dados onde cada campo é um valor simples — sem objetos aninhados, sem arrays profundos. Ex: `{ id: 1, nome: "Coca", categoria_nome: "Bebidas", preco: 5.99 }`. A RSdata só trabalha com dados planos. Se a fonte manda dado aninhado (`categoria: { nome: "Bebidas" }`), o Adapter "achata" antes de entregar ao Core.
...
### Plugin
Funcionalidade opcional que estende a RSdata sem inchar o Core. Vive nas bordas do projeto. Conecta-se ao Core via **ganchos oficiais** (hooks/eventos). Onde vivem as **dependências externas** (ex: lib de Excel). Exemplos: exportação CSV/Excel, seleção de linhas, cache, i18n.
...
### Escape hatch
"Alçapão de fuga" — um mecanismo oficial que permite ao usuário sair do caminho padrão (convenção) e ter controle total sobre algo específico, **sem abandonar todo o resto**. Na RSdata, os escape hatches existem em dois níveis: **peça** (customizar uma coluna) e **camada** (trocar o Theme inteiro).
...
Toda dependência externa que é **isolada** por uma interface interna (adapter) de forma que, se a dependência morrer ou for abandonada, **troca-se o adapter** sem afetar o resto do sistema. A RSdata não perde funcionalidade — só troca a peça. É o que garante a soberania.
...
### Soberania
Princípio fundador: a RSdata não depende de código de terceiros para funcionar. O Core é zero dependências. Dependências externas são exceções justificadas que vivem isoladas nas bordas. Se qualquer dependência morrer, a RSdata continua de pé.
...
### Dogfooding
"Comer a própria ração" — a prática de usar o próprio produto para resolver problemas reais. A RSdata nasce como dogfooding: o autor a usa no seu projeto DDD (Laravel → Nuxt) antes de oferecê-la ao público. Isso valida a lib em condições reais antes do lançamento OSS.
...
### Release limpo
Um release que **não carrega dívida técnica**. Toda solução temporária (caminho A) foi substituída pela correta (caminho B) antes do release sair. É a garantia de qualidade da RSdata: cada versão pública está arquiteturalmente íntegra.
...
### Action
Uma coluna especial que renderiza um **botão** (gatilho) em cada linha. A RSdata emite um evento com o dado da linha quando o botão é clicado. A lógica do que acontece depois (excluir, editar, navegar) é **100% responsabilidade do usuário** — a RSdata só fornece o gatilho. *"A RSdata é o transportador; você traz a arma."*
